...
# Сначала только файлы описания зависимостей: слой restore переиспользуется,
# пока не изменились ссылки на пакеты.
# .editorconfig нужен и в сборке: в нём настроены анализаторы,
# а предупреждения в этом решении трактуются как ошибки.
