...
/// <summary>Правило отбора часов для экрана: оставшиеся часы текущих суток плюс все часы следующих - это бизнес-правило из ТЗ, поэтому оно живёт в домене и покрыто тестами, а не растворено в разметке страницы</summary>
public static class HourlyWindow
{
    /// <summary>Отбирает часы окна относительно локального времени точки запроса</summary>
    ...
    /// <remarks>Текущий час включается: в 10:30 пользователь ожидает увидеть слот «10:00» как актуальный, а не считать его прошедшим</remarks>
