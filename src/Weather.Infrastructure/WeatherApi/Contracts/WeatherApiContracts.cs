...
/*
 * Контракты внешнего API. Имена свойств переводятся в snake_case
 * политикой именования сериализатора, поэтому атрибуты не нужны.
 * Все ссылочные поля объявлены nullable: провайдер вправе не прислать
 * любое из них, и маппер обязан это пережить.
 */
