...
{
    /// <summary>
    /// Имя типизированного клиента; используется тестами для подмены базового адреса.
    /// </summary>
        ...
        IConfiguration configuration)
    {
        ...
        services.AddOptions<WeatherApiOptions>()
            ...
            // Отсутствующий ключ должен ронять приложение на старте,
        ...
        services.AddHttpClient<WeatherApiClient>((serviceProvider, client) =>
            {
                // Адрес берётся из проверенных опций в момент создания клиента,
                // поэтому пользовательские секреты и переменные окружения,
                // подключённые после регистрации, тоже учитываются.
            ...
            // Стандартный обработчик устойчивости: повторы, размыкатель цепи,
            // таймаут попытки и общий таймаут запроса — вместо ручных политик Polly.
        ...
        {
            // Нулевое время жизни означает «кэш выключен» и обрабатывается в декораторе;
            // сам HybridCache нулевую длительность в настройках по умолчанию не принимает.
        ...
        // Доменные снимки собираются фабричными методами с проверками,
        // поэтому кэшу нужен явный сериализатор вместо соглашений по умолчанию.
        ...
        // Порт наружу отдаётся уже завёрнутым в кэш: вызывающий код
        // не знает и не должен знать о существовании кэширования.
